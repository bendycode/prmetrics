...
RSpec.describe RepositoriesController do
  ...
  describe 'POST #sync' do
    it 'queues a sync job' do
      ...
      end.to have_enqueued_job(UnifiedSyncJob).with(repository.name, fetch_all: false)
    end
    ...
    context 'with fetch_all parameter' do
      it 'queues a full sync job' do
        ...
        end.to have_enqueued_job(UnifiedSyncJob).with(repository.name, fetch_all: true)
      end
    end
  end
  ...
  describe 'POST #create' do
    context 'with valid params' do
      ...
      it 'queues a sync job' do
        expect do
          post :create, params: { repository: valid_attributes }
        end.to have_enqueued_job(UnifiedSyncJob).with('rails/rails', fetch_all: true)
      end
