...
RSpec.describe Week, type: :model do
  ...
  describe 'instance methods' do
    ...
    describe '#open_prs' do
      let!(:open_pr) { create(:pull_request, repository: repository, draft: false, gh_created_at: current_week.begin_date) }
      let!(:closed_pr) { create(:pull_request, repository: repository, draft: false, gh_created_at: current_week.begin_date, gh_closed_at: current_week.end_date) }
      ...
      it 'returns non-draft PRs that were open during the week' do
        expect(current_week.open_prs).to include(open_pr)
        expect(current_week.open_prs).not_to include(draft_pr)
        expect(current_week.open_prs).not_to include(closed_pr)
      end
    end
